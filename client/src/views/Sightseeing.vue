<template>
  <div>
    ...
    <div class="d-flex flex-wrap justify-center">
      <v-card v-for="sight in sights" :key="sight.id" class="mb-5 mr-5" max-width="344">
        ...
        <v-btn
          class="red--text ml-2 mb-2"
          plain
          color="primary"
          @click="getSight(sight.id)"
          :to="`/details/${sight.id}`"
          >Details</v-btn
        >
          ...
          <template v-slot:activator="{ on, attrs }">
            <v-btn
              @click="
                edited = item;
                title = edited.title;
                admission = edited.admission;
              "
              class="red--text ml-auto mb-2"
              plain
              color="warning"
              v-bind="attrs"
              v-on="on"
          ...
          <template v-slot:default="dialog">
            <v-card>
              ...
              <v-form class="mr-5 ml-5">
                <v-text-field v-model="title" label="Title" value="sight.title" required></v-text-field>
                <v-text-field v-model="admission" label="Admission" required></v-text-field>
              </v-form>
              {{ edited }}
              <v-card-actions>
                <v-btn color="blue" text @click="(dialog.value = false), patchSight(sight.id)">Update</v-btn>
                <v-btn class="ml-auto" color="success" text @click="dialog.value = false">Beenden</v-btn>
              </v-card-actions>
            </v-card>
          </template>
